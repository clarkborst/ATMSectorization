...
The evolutionary algorithm explores the solution space and presents a rich collection of alternatives at every optimization run. These solutions are displayed to the human operator through an interactive Pareto frontier, where the user can visually inspect trade-offs across objectives and indicate preferred or rejected solutions with minimal cognitive effort. Each feedback action—selecting a promising configuration, rejecting a fragmented sector, or choosing a geometry that better aligns with expected traffic flows—provides a high-value learning signal.
